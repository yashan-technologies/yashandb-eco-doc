SqlSugar是一款开源的关系映射器（ORM）框架，适用于.NET的对象，该框架封装了对数据库的各项操作，使开发者可以直接用.NET而非SQL语句来操作数据库。本文将介绍如何实现YashanDB（yashan模式）与SqlSugar的对接适配，从而让用户通过.NET语言连接和操作YashanDB。

## 对接前准备

在进行对接操作前，您需要先准备好如下事项：

- 已安装.NET core 2.1应用环境。

- 已安装Visual Studio工具。

- 已在[YashanDB官网下载中心](https://download.yashandb.com/download)下载并安装YashanDB ODBC驱动，其中是否配置数据源将影响应用项目的数据库连接操作，请根据实际需要选择。

   - 如您的应用项目还需进行ODBC开发，则必须配置数据源，后续应用开发中按DSN格式连接YashanDB

   - 如您的应用项目无需进行ODBC开发，则可以不配置数据源，后续应用开发中按Driver格式连接YashanDB

- 已向我们的技术支持人员获取SqlSugar-yashan适配包。
...
3. 在弹出窗口中选择【程序包源】，单击【+】新建一个NuGet包源，为其命名，例如SqlSugar，路径请选择SqlSugar.5.1.4.197.nupkg路径。
4. 单击Visual Studio的【解决方案资源管理器】，选中您的应用项目，右键弹出菜单中选择【管理Nuget程序包】，在【已安装的包】列表中选择SqlSugar。
5. 单击【安装】，成功安装后即完成应用项目对SqlSugar.5.1.4.197.nupkg的引入。
