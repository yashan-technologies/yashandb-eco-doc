Django是一个免费开源的Python Web开发框架，内部包含了数据库ORM组件。django_yashandb是适配Django ORM框架的YashanDB方言库，且已通过Django社区用例集。本文将对介绍如何通过django_yashandb实现YashanDB（yashan模式）与Django的对接。

## 对接准备

在进行对接操作前，您需要先准备好如下事项：

- 已安装python3.8以上版本的Python应用环境。

- 已安装Django 3.2.x，安装命令如下：

    ```shell
    pip3 install django==3.2.25
    ```

- 已在[YashanDB官网下载中心](https://download.yashandb.com/download)下载并安装YashanDB Python驱动包。

- 已向我们的技术支持人员获取django_yashandb方言包。
...
- **请勿**手动为AutoField、BigAutoField插入与当前数据库中该字段最大值跨度过大的值，这可能会导致框架长时间阻塞。
