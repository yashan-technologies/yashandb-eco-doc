Entity Framework 6（EF6）是由微软公司提供的一款关系映射器（ORM），适用于 .NET 的对象，该框架封装了对数据库的各项操作，使开发者可以直接用.NET而非SQL语句来操作数据库。本文将介绍如何实现YashanDB（yashan模式）与EF6的对接，从而让用户通过.NET语言连接和操作YashanDB。

## 对接前准备

在进行对接操作前，您需要先准备好如下事项：

- 已安装.NET应用环境，版本要求为.net framework 4.5、4.5.1、4.5.2以及6.0中的某一项。
...
2. 使用dotnet命令指定Yashandb.Data.EntityFramework文件夹所在路径，将Yashandb.Data.EntityFramework项目添加到该应用程序项目内。
