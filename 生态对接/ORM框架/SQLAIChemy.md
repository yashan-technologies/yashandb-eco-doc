SQLAlchemy是一款开源的Python SQL工具包和对象关系映射器（ORM），yashandb_sqlalchemy是适配SQLAlchemy框架的YashanDB方言库，已通过SQLAlchemy社区用例集。本文将对介绍如何通过yashandb_sqlalchemy实现YashanDB（yashan模式）与SQLAlchemy的对接。

## 对接准备

在进行对接操作前，您需要先准备好如下事项：

- 已安装python3以上版本的Python应用环境。

- 已安装SQLAlchemy 1.4.x，安装命令如下：
...
- 已向我们的技术支持人员获取yashandb_sqlalchemy方言包。
