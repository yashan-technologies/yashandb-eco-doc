...
4. 文件添加后，上图中的【找到类】将变的可用，单击它，从弹出的驱动类中选择`com.yashandb.jdbc.Driver`。
...
6. 单击【确定】，至此YashanDB JDBC驱动已成功对接到DBeaver工具中。
...
2. 在弹出窗口中，下拉或搜索找到对接配置中新建的驱动名称并选中。

   ![](./image/dbeaver4.png)

3. 单击【下一步】，进入连接信息配置页面，输入如下信息：
...
4. 单击【测试连接】，输出成功连接信息时，表示连接设置成功。
5. 单击【完成】退出连接设置窗口，在【数据库导航】界面将会看到新建的连接，下拉它，成功展示YashanDB相关信息说明已成功连接到YashanDB，开发者可以开始输入SQL语句操作YashanDB。
